...
const NavLink = ({ href, name }: { href:string, name:string }) => {
    return (
        <Link href={href} title={name} className="text-sm text-[var(--foreground)] transition-all duration-200 border-transparent border-b-[1px] hover:border-[var(--foreground)] focus:border-[var(--foreground)] w-fit uppercase tracking-widest">
            {name}
        </Link>
    )
}
...
const SliderNavLink = ({ href, name, setIsOpenMenuSlider }: { href:string, name:string, setIsOpenMenuSlider: Dispatch<SetStateAction<boolean>> }) => {
    return (
        <Link href={href} title={name} className="text-sm text-[var(--foreground)] transition-all duration-200 border-transparent border-b-[1px] hover:border-[var(--foreground)] focus:border-[var(--foreground)] w-fit tracking-widest uppercase" onClick={(e) => {
            e.preventDefault();
    ...
    return (
        ...
        <header className="sticky top-0 shadow-2xl bg-[var(--background)] z-50 font-poppins">
            <div className="px-4 mx-auto max-w-[105rem] sm:px-6 lg:px-8">
                <nav className="flex items-center justify-between py-5 pb-7">
                    <div className="flex items-center justify-center gap-3">
                        <Link href={"#"} className="lg:hidden cursor-pointer mt-2" onClick={(e) => {
                                e.preventDefault();
                        ...
                        <div className="relative w-8 h-8">
                            ...
                            {isMiniCartShow && (
                                <div className="absolute top-9 right-0 shadow-2xl rounded-xl">
                                    <div className="bg-[var(--foreground)] rounded-xl p-4 md:p-8 text-[var(--background)] overflow-hidden min-w-80 md:min-w-xl before:content-[''] before:absolute before:w-4 before:h-4 before:-top-1 before:right-2 before:bg-[var(--foreground)] before:rotate-45">
                                        ...
                                        <div className="mt-4 md:mt-8">
                                            <Link href={"/checkout"} role="button" title="Checkout" className="bg-[var(--background)] flex gap-2 justify-center items-center text-[var(--foreground)] py-4 rounded-lg w-full text-center focus:outline-none focus:ring-2 focus:ring-yellow-100 focus:ring-offset-2">
                                                {"Go To Checkout"}
                                            </Link>
                                        </div>
                                    </div>
        ...
        <div className={isOpenMenuSlider ? `lg:hidden font-poppins shadow-2xl fixed top-0 left-0 min-h-dvh min-w-3/4 bg-[var(--background)] z-50 translate-x-0 transition-all border-r-1 border-[var(--foreground)]` : `lg:hidden font-poppins shadow-2xl fixed top-0 left-0 min-h-dvh min-w-3/4 bg-[var(--background)] z-50 -translate-x-[100%] transition-all border-r-1 border-[var(--foreground)]`}>
            <div className="p-5 relative">
                <Link href={"/"} title="Pistorium" className="absolute top-4 right-4 flex border-2 rounded-full p-2" onClick={(e) => {
                        e.preventDefault();
                        setIsOpenMenuSlider(false)
                }}>
                    <svg viewBox="0 0 24 24" className="text-[var(--foreground)] w-3 h-3" fill="none" xmlns="http://www.w3.org/2000/svg"><g strokeWidth="0"></g><g strokeLinecap="round" strokeLinejoin="round"></g><g> <path d="M20.7457 3.32851C20.3552 2.93798 19.722 2.93798 19.3315 3.32851L12.0371 10.6229L4.74275 3.32851C4.35223 2.93798 3.71906 2.93798 3.32854 3.32851C2.93801 3.71903 2.93801 4.3522 3.32854 4.74272L10.6229 12.0371L3.32856 19.3314C2.93803 19.722 2.93803 20.3551 3.32856 20.7457C3.71908 21.1362 4.35225 21.1362 4.74277 20.7457L12.0371 13.4513L19.3315 20.7457C19.722 21.1362 20.3552 21.1362 20.7457 20.7457C21.1362 20.3551 21.1362 19.722 20.7457 19.3315L13.4513 12.0371L20.7457 4.74272C21.1362 4.3522 21.1362 3.71903 20.7457 3.32851Z" fill="currentColor"></path> </g></svg>
                </Link>
                ...
                <h4 className="text-xl font-extrabold tracking-widest border-b-1 text-[var(--foreground)] mt-8 mb-4 pb-2 pl-2">{"Follow Us"}</h4>
                <div className="flex space-x-5 justify-start items-center mb-14 ml-2">
                    <Link href="https://www.instagram.com/pistorium.official/" target="_blank" className="block text-[var(--foreground)] transition-all duration-500">
                    ...
                    <Link href="https://facebook.com" target="_blank" className="block text-[var(--foreground)] transition-all duration-500">
                    ...
                    <Link href="https://youtube.com" target="_blank" className="block text-[var(--foreground)] transition-all duration-500">
